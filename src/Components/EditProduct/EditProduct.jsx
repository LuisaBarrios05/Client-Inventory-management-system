...
const EditProduct = () => {
      ...
      try {
        ...
        setFormData({
          ...
          medidas: {
            alto: medidas.altura || '',
            ancho: medidas.ancho || '',
            profundidad: medidas.profundidad || ''
          },
  ...
  const formatPrice = (price) => {
    ...
    if (lastDotIndex !== -1 && price.length - lastDotIndex - 1 > 2) {
      // Si hay más de dos dígitos después del último punto decimal, eliminar todos los puntos
  ...
  const handleSubmit = async (e) => {
    ...
    if (validate()) {
      ...
      try {
        ...
        console.log("imagen_url:",formData.image)
        if (!image || image == null){
          formDataToSubmit.append('imagen', formData.image);
        }else{
        ...
        formDataToSubmit.append('precio', formatPrice(formData.price.toString()));

        const response = await axios.put(`${ApiUrl}/update-product/${id}`, formDataToSubmit, {
          headers: {
    ...
    <div className="flex justify-center items-center mt-5">
      <div className="bg-white rounded-lg shadow-lg p-7 max-w-[620px] h-auto">
        <div className="text-center mb-4">
          <p className="text-2xl font-bold">Editar Producto</p>
        </div>
        ...
        <div className="text-center mt-5 flex justify-center space-x-4">
          <button type="submit" className="bg-black hover:bg-black text-white font-semibold py-2 px-16 rounded-lg" onClick={handleSubmit}>
            Guardar cambios
          </button>
